...
def buil_model() -> Model:
    ...
    x = Dense(units=10, activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(x)
    x = Dropout(rate=0.05)(x)
    x = Dense(units=10, activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(x)
    x = Dense(units=10, activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(x)

    binary_output = Dense(units=1, activation='sigmoid', name='binary_output')(x)

    another_layer = Dense(units=10, activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(x)
    multiclass_output = Dense(units=6, activation='sigmoid', name='multiclass_output')(another_layer)
...
def run_multiclass_multilabel_model_tuning(config: Dict) -> None:
    ...
    # Print Metrics
    plot_multiple_learning_curves(history=history.history)

    # Test Binary Model
    y_val = data_dict['y_val'][0]
    y_pred = model.predict(x_val_scaled)[0]
    y_pred_thresholded_labels = (y_pred > THRESHOLD_3).astype('int')
    cm = confusion_matrix(y_val, y_pred_thresholded_labels, labels=[0,1])
    plot_confusion_matrix(cm=cm, labels=BINARY_LABELS, name='Binary')
    ...
    plot_multiple_confusion_matrices(cms=cm, labels=MULTICLASS_LABELS, name='Multiclass')
    ...
    cm = confusion_matrix(y_val, y_pred_thresholded_labels, labels=[0,1])
    plot_confusion_matrix(cm=cm, labels=BINARY_LABELS, name='Test_Binary_1')
    # Threshold 2
    y_pred_thresholded_labels = (y_pred > THRESHOLD_2).astype('int')
    cm = confusion_matrix(y_val, y_pred_thresholded_labels, labels=[0,1])
    plot_confusion_matrix(cm=cm, labels=BINARY_LABELS, name='Test_Binary_2')
    # Threshold 3
    y_pred_thresholded_labels = (y_pred > THRESHOLD_3).astype('int')
    cm = confusion_matrix(y_val, y_pred_thresholded_labels, labels=[0,1])
    plot_confusion_matrix(cm=cm, labels=BINARY_LABELS, name='Test_Binary_3')
    ...
    for thresh in [0.05, 0.10, 0.15, 0.20, 0.25, 0.30]:
        compute_multiclass_multilabel_metrics(y_target=y_val,
                                              ...
                                              threshold=thresh,
                                              file_name=f'Test_Multiclass_Architecture0_Metrics.csv')
    cm = multilabel_confusion_matrix(y_val, y_pred_val)
    plot_multiple_confusion_matrices(cms=cm, labels=MULTICLASS_LABELS, name='Test_Multiclass')

    model.save('multioutpput_model.h5')
